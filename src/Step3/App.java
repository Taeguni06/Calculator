    ...
    static void main() {
        ...
        while(true) {
            try {
                ...
                if (action == 1) {
                    System.out.println("[정수 계산기] 숫자 두 개와 연산자를 입력하세요 기록 조회: history 기록 삭제: remove 탐색: find 수정: fix 종료: exit");
                    Calculator.runCalculator(sc, intCal, Integer::valueOf);
                } else if (action == 2) {
                    System.out.println("[실수 계산기] 숫자 두 개와 연산자를 입력하세요 기록 조회: history 기록 삭제: remove 탐색: find 수정: fix 종료: exit");
